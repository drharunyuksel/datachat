...
export class VannaChat extends LitElement {
  static styles = [
    ...
    css`
      /* Schema Sidebar */
      .chat-layout.has-schema-sidebar {
        grid-template-columns: 280px minmax(0, 1fr) 300px;
      }

      .chat-layout.compact.has-schema-sidebar {
      ...
      .schema-header {
        ...
        border-bottom: 1px solid var(--chat-outline);
        display: flex;
        flex-direction: column;
        gap: var(--vanna-space-2);
        flex-shrink: 0;
      ...
      .schema-group-label {
        padding: var(--vanna-space-1) var(--vanna-space-4);
        font-size: 0.7rem;
        font-weight: 600;
        color: var(--vanna-foreground-dimmest);
        text-transform: uppercase;
  ...
  @state() private expandedTables: Set<string> = new Set();
  ...
  private async toggleSchemaSidebar() {
    this.schemaSidebarOpen = !this.schemaSidebarOpen;
    if (this.schemaSidebarOpen && this.schemaData.length === 0) {
  ...
  private toggleTableExpand(key: string) {
    const newSet = new Set(this.expandedTables);
    if (newSet.has(key)) {
      newSet.delete(key);
    } else {
      newSet.add(key);
    }
    this.expandedTables = newSet;
  }

  private insertIntoChat(text: string) {
    this.currentMessage = this.currentMessage ? `${this.currentMessage} ${text}` : text;
  ...
  private handleSchemaFilter(e: Event) {
    this.schemaFilter = (e.target as HTMLInputElement).value.toLowerCase();
  }
      ...
      <div class="schema-sidebar">
        <div class="schema-header">
          <div class="schema-header-title">
            ...
            <button class="schema-close-btn" @click=${this.toggleSchemaSidebar} title="Close">
          ...
          <input
            ...
            .value=${this.schemaFilter}
            @input=${this.handleSchemaFilter}
          />
        </div>
        <div class="schema-tree">
          ${this.schemaLoading ? html`
            <div class="schema-loading">Loading schema...</div>
          ` : this.schemaData.map(schema => {
            const filteredTables = schema.tables.filter((t: any) =>
              !filter || t.name.toLowerCase().includes(filter) ||
              t.columns.some((c: any) => c.name.toLowerCase().includes(filter))
            );
            if (filteredTables.length === 0) return '';
            return html`
              <div class="schema-group-label">${schema.name}</div>
              ${filteredTables.map((table: any) => {
                const key = `${schema.name}.${table.name}`;
                const expanded = this.expandedTables.has(key);
                return html`
                  <div class="schema-table-item" @click=${() => this.toggleTableExpand(key)}>
                    <span class="schema-table-icon">${expanded ? '▼' : '▶'}</span>
                    <span class="schema-table-name" title=${table.name}
                      @dblclick=${(e: Event) => { e.stopPropagation(); this.insertIntoChat(table.name); }}>
                      ${table.name}
                    </span>
                  ...
                  ${expanded ? html`
                    <div class="schema-columns">
                      ${table.columns
                        .filter((c: any) => !filter || c.name.toLowerCase().includes(filter) || table.name.toLowerCase().includes(filter))
                        .map((col: any) => html`
                        <div class="schema-column-item"
                          @click=${(e: Event) => { e.stopPropagation(); this.insertIntoChat(col.name); }}
                          title="${col.name} (${col.type})${col.nullable ? ' - nullable' : ''}">
                          <span class="schema-table-name">${col.name}</span>
          ...
          <div class="chat-header">
            <div class="header-top">
              ...
              <div class="header-top-actions">
                <button
                  class="schema-toggle-btn ${this.schemaSidebarOpen ? 'active' : ''}"
                  @click=${this.toggleSchemaSidebar}
                  title="${this.schemaSidebarOpen ? 'Hide schema' : 'Show schema'}">
                  <svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16">
                    <path d="M4 3h16c1.1 0 2 .9 2 2v4c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V5c0-1.1.9-2 2-2zm0 8h16c1.1 0 2 .9 2 2v4c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2v-4c0-1.1.9-2 2-2zm2-5v2h2V6H6zm0 8v2h2v-2H6z"/>
                  </svg>
                  Tables
                </button>
